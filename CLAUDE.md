...
**Boss detail works the same way, from `public/boss-detail/`.** 147 files,
~566 KB, generated by `build:bosses` alongside `src/data/bosses.json` — drop
tables plus the wiki's "Fight overview" prose. Another `diaries.json` of
precache to ship every player 147 boss pages they read one at a time, so same
`globIgnores` array, same runtime-rule treatment.

Named `boss-detail`, not `drops`, because it stopped being only drops within
the hour. What is deliberately **not** in it: the `<boss>/Strategies` subpages,
at 34–47 KB of gear setups and tables each, roughly 4 MB across the set. That
...
**There is deliberately no pinned precache entry count or byte budget.** There
was one, and it was wrong three times running for reasons that shipped nothing
new: 58 → 62 when the `/sync` view made `sync`, `persist` and `api` shared
chunks, then → 60 when the Diaries panel was deleted and `DiaryList` folded
...
precache bytes with real headroom) rather than an entry count.
